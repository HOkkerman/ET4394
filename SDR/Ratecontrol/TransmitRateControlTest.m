...
%% Introduction
% The IEEE(R) 802.11 standard supports dynamic rate control by adjusting
% the MCS value of each transmitted packet based on the underlying radio
% propagation channel. Maximizing link throughput, in a propagation channel
% that is time varying due to multipath fading or movement of the
% surrounding objects, requires dynamic variation of MCS. The IEEE 802.11
% standard does not define any standardized rate control algorithm (RCA)
% for dynamically varying the modulation rate. The implementation of RCA is
% left open to the WLAN device manufacturers. This example uses a
% closed-loop rate control scheme. A recommended MCS for transmitting a
% packet is calculated at the receiver and is available at the transmitter.
% In a real system this information would be conveyed through a control
% frame exchange. The MCS is adjusted for each subsequent packet in
% response to an evolving channel condition with noise power varying over
% time.
%
% In this example, an IEEE(R) 802.11ac [ <#10 1> ] waveform consisting of
