...
avg1=zeros(length(bandwidth), length(delay_profile));
avg2=zeros(length(bandwidth), length(delay_profile));
avg3=zeros(length(bandwidth), length(delay_profile));

%mov1=zeros(length(bandwidth), length(delay_profile));
%mov2=zeros(length(bandwidth), length(delay_profile));
%mov3=zeros(length(bandwidth), length(delay_profile));
...
for bw=2:length(bandwidth)
    for dp=1:length(delay_profile)
        ...
        [avg1(bw, dp), mov1] = ratecontrol_old(npackets, bandwidth_in, delay_profile_in, distance_in);
        [avg2(bw, dp), mov2] = ratecontrol(npackets, bandwidth_in, delay_profile_in, distance_in, historysize);
        [avg3(bw, dp), mov3] = ratecontrol_weighted(npackets, bandwidth_in, delay_profile_in, distance_in, weights);    
    end
